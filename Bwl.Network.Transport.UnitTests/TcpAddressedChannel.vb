...
<TestClass()> Public Class TcpAddressedTransportTests
    <TestMethod()> Public Sub TAT_ConnectionsAddressed()
        ...
        AddHandler server.Server.NewConnection, Sub(s As IPacketPortListener, t As IConnectedChannel)
                                                    connectedList.Add(t)
                                                End Sub
        AddHandler server.RegisterClientRequest, Sub(clientInfo As Dictionary(Of String, String), id As String, method As String, password As String, serviceName As String, options As String, ByRef allowRegister As Boolean, ByRef infoToClient As String)
                                                     allowRegister = True
                                                 End Sub
        server.Server.Open("localhost:3043", "")
        client1.Channel.Open("localhost:3043", "")
        client2.Channel.Open("localhost:3043", "")
        Threading.Thread.Sleep(100)
        client1.RegisterMe("Client 1", "", "Service 1", "")
        client2.RegisterMe("Client 2", "", "Service 2", "")
        If connectedList.FindAll(Function(t As IConnectedChannel) t.Channel.ID = client1.Channel.ID).Count = 1 Then Throw New Exception("Client 1 not received in server.NewConnection")
        If connectedList.FindAll(Function(t As IConnectedChannel) t.Channel.ID = client2.Channel.ID).Count = 1 Then Throw New Exception("Client 2 not received in server.NewConnection")
        If server.Server.ActiveConnections.Count(Function(t As IConnectedChannel) t.Channel.ID = client1.Channel.ID) = 1 Then Throw New Exception("Client 1 not received in server.NewConnection")
        If server.Server.ActiveConnections.Count(Function(t As IConnectedChannel) t.Channel.ID = client2.Channel.ID) = 1 Then Throw New Exception("Client 2 not received in server.NewConnection")
    ...
    <TestMethod()> Public Sub TAT_AddressedSend()
        ...
        client2.Channel.Open("localhost:3044", "")
        Threading.Thread.Sleep(100)
        client1.RegisterMe("Client 1", "", "Service 1", "")
        client2.RegisterMe("Client 2", "", "Service 2", "")
