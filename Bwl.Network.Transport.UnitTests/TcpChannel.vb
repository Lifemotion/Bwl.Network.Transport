...
<TestClass()> Public Class TcpTransportTests
    ...
    <TestMethod()> Public Sub TT_Connections()
        ...
        Dim client1 As New TCPChannel()
        Dim client2 As New TCPChannel()
    ...
    <TestMethod()> Public Sub TT_SendReceiveAsync()
        ...
        AddHandler server.ServerReceivedPacket, Sub(t As IConnectedChannel, p As BytePacket)
                                                    serverReceived = p
                                                End Sub
        AddHandler server.ServerSentPacket, Sub(t As IConnectedChannel, p As BytePacket)
                                                serverSent = p
                                            End Sub
    ...
    <TestMethod()> Public Sub TT_SendReceive()
        ...
        AddHandler server.ServerSentPacket, Sub(t As IConnectedChannel, p As BytePacket)
                                                serverSent = p
                                            End Sub
        AddHandler client1.PacketReceived, Sub(transport As IPacketChannel, p As BytePacket)
                                               client1Received = p
                                           End Sub
        AddHandler client1.PacketSent, Sub(transport As IPacketChannel, p As BytePacket)
                                           client1Sent = p
                                       End Sub
        ...
        server.ActiveConnections(0).Channel.SendPacket(bp2)
